document.addEventListener("DOMContentLoaded", () => {
        ...
        const participantsList = details.participants.length > 0
          ? `<div class="participants-list">${details.participants.map(p => `
            <div class="participant-item">
              <span>${p}</span>
              <button class="delete-participant" data-participant="${p}" data-activity="${name}" title="Remove participant">×</button>
            </div>
          `).join('')}</div>`
  ...
  async function handleDeleteParticipant(event) {
    ...
    try {
      ...
      if (response.ok) {
        messageDiv.textContent = `${participant} has been removed from ${activity}`;
